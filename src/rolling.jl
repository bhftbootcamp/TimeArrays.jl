...
Applies function `f` in a sliding window with window size `n` to the elements of `t_array`.
...
function ta_rolling(
    ...
    for i in 1:len
        ...
        interval = max(i - window + 1, 1):i
        v = if length(interval) < observations
            ta_nan(V2)
        else
            f(view(values, interval))
        end
...
Applies function `f` in a sliding window with window period `p` to the elements of `t_array`.
Function `f` must accept a vector with elements of type `V` as input and return a new value, which will be assigned to the corresponding timestamp of each window.
...
function ta_rolling(
    ...
    for r_index = len:-1:1
        ...
        interval = (l_index+1):r_index
        value = if l_index == -1 || length(interval) < observations
            ta_nan(V2)
        else
            l_index = l_index == 0 ? -1 : l_index
            f(view(values, interval))
        end
...
```
"""
function ta_sma(t_array::TimeArray{T,V}, window::Union{Integer,Period}; kw...) where {T,V}
    return ta_rolling(mean, t_array, window; kw...)
end
...
function ta_ema(t_array::TimeArray{T,V}, window::Integer) where {T,V}
    ...
    return ta_rolling(t_array, window; observations = 1) do slice
...
```
"""
function ta_wma(t_array::TimeArray{T,V}, window::Union{Integer,Period}; kw...) where {T,V}
    return ta_rolling(t_array, window; kw...) do slice
